...
impl Resolver {
    pub fn new(interpreter: Interpreter) -> Resolver {
        Resolver { interpreter, scopes: RefCell::new(Vec::new()) }
    }
}
...
impl ExprVisitor<()> for Resolver {
    ...
    fn visit_variable_expr(&self, wrapper: &Expr, expr: &VariableExpr) -> Result<(), LoxResult> {
        if !self.scopes.borrow().is_empty() && 
            !self.scopes.borrow().last().unwrap().borrow().get(expr.name.as_string().into()).unwrap() {
...
impl Resolver {
    ...
    fn resolve_local(&self, expr: &Expr, name: &Token) {
        for (scope_level, map) in self.scopes.borrow().iter().rev().enumerate() {
            if map.borrow().contains_key(name.as_string().into()) {
                self.interpreter.resolve(expr, scope_level);
                return;
            }
        }
    }
